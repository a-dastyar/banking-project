...
<nav class="d-flex justify-content-center" aria-label="Page navigation pagination">
    <c:if test="${page.total()>page.size()}">
        ...
        <ul class="pagination">
            ...
            <a class="page-link" href="${pageContext.request.contextPath}/${endpoint}?${pageParamName}=1&size=${page.size()}${typeParam}${additionalParam}" aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
            </a>
            </li>
            <c:forEach begin="${start}" end="${end}" varStatus="loop">
                <li class="page-item ${page.page()==loop.index?'active':''}">
                    <a class="page-link" href="${pageContext.request.contextPath}/${endpoint}?${pageParamName}=${loop.index}&size=${page.size()}${typeParam}${additionalParam}" >${loop.index}</a>
                </li>
            </c:forEach>
            <li class="page-item ${next==page.page()?'disabled':''}"">
            <a class="page-link" href="${pageContext.request.contextPath}/${endpoint}?${pageParamName}=${last}&size=${page.size()}${typeParam}${additionalParam}" aria-label="Next">
